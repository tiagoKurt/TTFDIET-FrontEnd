...
export class MetasService {
    ...
    listarMetas(): Observable<Meta[]> {
        return this._apiService.get<Meta[]>('/api/metas/').pipe(
            catchError((error) => {
    ...
    buscarMetaPorId(id: number): Observable<Meta> {
        return this._apiService.get<Meta>(`/api/metas/${id}`).pipe(
            catchError((error) => {
    ...
    criarMeta(metaRequest: MetaRequest): Observable<Meta> {
        return this._apiService.post<Meta>('/api/metas/', metaRequest).pipe(
            catchError((error) => {
    ...
    atualizarMeta(id: number, metaRequest: MetaRequest): Observable<Meta> {
        return this._apiService.put<Meta>(`/api/metas/${id}`, metaRequest).pipe(
            catchError((error) => {
    ...
    excluirMeta(id: number): Observable<void> {
        return this._apiService.delete<void>(`/api/metas/${id}`).pipe(
            catchError((error) => {
    ...
    atualizarProgresso(id: number, valorAtual: number): Observable<Meta> {
        return this._apiService
            .post<Meta>(`/api/metas/${id}/atualizar-progresso`, { valorAtual })
            .pipe(
                catchError((error) => {
    ...
    obterResumoMetas(): Observable<any> {
        return this._apiService.get<any>('/api/metas/resumo').pipe(
            catchError((error) => {
    ...
    obterProgressoMetas(): Observable<any> {
        return this._apiService.get<any>('/api/metas/progresso').pipe(
            catchError((error) => {
                ...
                metas.forEach((meta) => {
                    ...
                    if (meta.tipoMeta === 'CALORIAS' && progresso > 90) {
                        recomendacoes.push({
                            ...
                            descricao:
                                'Parabéns! Você está muito próximo da sua meta calórica diária.',
                            meta: meta,
                        });
                    }
                });
